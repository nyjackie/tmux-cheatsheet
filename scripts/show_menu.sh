...
# cmd  <key> <description> <prefix>   -> label: green keycap + description + peach prefix ref
# The same <key> is also passed as the menu mnemonic (the field after the label),
# so pressing it runs the item; tmux additionally shows it as "(key)" on the right.
# Note: this tmux build counts #[...] markup toward the row width, so on very
# narrow terminals a ">" truncation marker can appear; at normal widths it won't.
cmd()  { printf '%s %s %s %s%-19s%sprefix %s%s' "$CHIP" "$1" "$RST" "$DESC" "$2" "$REF" "$3" "$RST"; }
...
menu_misc() {
  "${MENU[@]}" -T "#[fg=#cba6f7,bold] Copy mode & misc — click to run " \
    ...
    "$(cmd k 'All keybindings' '?')" k "list-keys" \
    "" \
    "${DIM}◀ Back${RST}" Left "run-shell '$SELF'"
}
